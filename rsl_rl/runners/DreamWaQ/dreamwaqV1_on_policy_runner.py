...
class DreamWaQV1OnPolicyRunner:
        ...
        for it in range(start_it, total_it):
            ...
            with torch.inference_mode():
                for _ in range(self.cfg["num_steps_per_env"]):
                    ...
                    obs, reconstructed_obs_targets, rewards, dones, extras = self.env.step(actions.to(self.env.device))
                    # Move to device
                    obs, reconstructed_obs_targets, rewards, dones = (obs.to(self.device), reconstructed_obs_targets.to(self.device), rewards.to(self.device), dones.to(self.device))
                    # Process the step
                    self.alg.process_env_step(obs, reconstructed_obs_targets, rewards, dones, extras)
