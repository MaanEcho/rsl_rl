...
from rsl_rl.modules import ActorCriticRecurrent, ActorCriticDreamWaQ
...
class PPODreamWaQV1:
    ...
    policy: ActorCriticRecurrent | ActorCriticDreamWaQ
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        if kwargs:
            print(
                "PPODreamWaQ.__init__ got unexpected arguments, which will be ignored: " + str([key for key in kwargs])
            )
    ...
    def process_env_step(
        self, obs: TensorDict, reconstructed_obs_targets: torch.Tensor, rewards: torch.Tensor, dones: torch.Tensor, extras: dict[str, torch.Tensor]
    ) -> None:
        ...
        self.transition.reconstructed_obs_targets = reconstructed_obs_targets
    ...
    def update(self, current_iteration: int, total_iterations: int) -> dict[str, float]:
        ...
        for (
            ...
            vel_est_loss = nn.functional.mse_loss(encode_lin_vel_normalized_batch, lin_vel_targets_normalized_batch)
            reconstruction_loss = nn.functional.mse_loss(reconstructed_obs_normalized_batch, reconstructed_obs_targets_normalized_batch)
            latent_loss = torch.mean(-0.5 * torch.sum(1 + context_logvar_batch - context_mean_batch.square() - context_logvar_batch.exp(), dim=-1))

            cenet_loss = vel_est_loss + reconstruction_loss + self.beta * latent_loss
            ...
            mean_vae_loss += (cenet_loss.item() - vel_est_loss.item())
