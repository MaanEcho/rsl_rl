...
class RolloutStoragePIE:
    ...
    def recurrent_mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator:
        ...
        for ep in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                lin_vel_targets_batch = self.observations["privileged"][:, start:stop, :3]
                feet_clearances_targets_batch = self.observations["privileged"][:, start:stop, 3:7]
                height_scan_reconstruction_targets_batch = self.observations["privileged"][:, start:stop, 7:]
                obs_reconstruction_targets_batch = self.next_observations["proprioception"][:, start:stop]
                dones_batch = self.dones[:, start:stop].squeeze(-1)
    ...
    def _save_hidden_states(self, hidden_states: HiddenState) -> None:
        if hidden_states is None:
            return
        # Make a tuple out of GRU hidden states to match the LSTM format
        hidden_state_a = hidden_states if isinstance(hidden_states, tuple) else (hidden_states,)
