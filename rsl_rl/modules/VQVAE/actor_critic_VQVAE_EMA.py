...
class ActorCriticVQVAEEMA(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        num_actor_obs += cenet_cfg["codebook_dim"]
    ...
    def act(self, obs: TensorDict, bootstrap: bool, stage: Literal["rollout", "update"], **kwargs: dict[str, Any]) -> torch.Tensor | tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if stage == "rollout":
            ...
            return encode_lin_vel, z_q_st, z_e, z_q
        else:
    ...
    def _export_policy_as_jit(self, model: nn.Module, path: Path) -> None:
        ...
        export_path = path.with_name(f"policy_{checkpoint}_VQVAE.pt")
        with torch.no_grad():
    ...
    def _export_policy_as_onnx(self, obs: TensorDict, model: nn.Module, path: Path) -> None:
        path.parent.mkdir(parents=True, exist_ok=True)
        checkpoint = path.stem.split("_")[-1]
        export_path = path.with_name(f"policy_{checkpoint}_VQVAE.onnx")
        with torch.no_grad():
            policy = copy.deepcopy(model).to("cpu")
            policy.eval()
            for p in policy.parameters():
...
class CENet(nn.Module):
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        decoder_input_dim = codebook_dim
        ...
        for key in encoder_head_hidden_dims:
            if encoder_head_hidden_dims[key] is not None:
                setattr(self, f"encoder_head_{key}", MLP(encoder_output_dim, codebook_dim if key == "codebook" else estimated_state_dims[key], encoder_head_hidden_dims[key], activation))
            else:
                setattr(self, f"encoder_head_{key}", nn.Linear(encoder_output_dim, codebook_dim if key == "codebook" else estimated_state_dims[key]))
    ...
    def decode(self, estimated_states: torch.Tensor, states: torch.Tensor, bootstrap: bool, z_q_st: torch.Tensor) -> torch.Tensor:
        if bootstrap:
            return self.decoder(torch.cat((estimated_states.detach(), z_q_st), dim=-1))
        else:
    ...
    def _quantize(self, z_e: torch.Tensor, update_ema: bool = False) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Returns:
            ...
            z_q_st: (..., D) straight-through vectors (forward == z_q, backward flows to z_e).
            indices: (...) int64 code indices.
        """
        if z_e.shape[-1] != self.codebook.embedding_dim:
            raise ValueError(
                f"z_e last-dim ({z_e.shape[-1]}) != codebook_dim ({self.codebook.embedding_dim})"
            )

        # Flatten leading dims so we can do (N, D) x (K, D)
        ...
        z_q_st = z_q_st_flat.reshape(*z_e.shape)
        indices = indices_flat.reshape(*z_e.shape[:-1])
...
class InferenceWrapper(nn.Module):
    def __init__(self, models: ActorCriticVQVAEEMA) -> None:
        ...
        self.encoder_head_codebook = models.cenet.encoder_head_codebook
        self.codebook = models.cenet.codebook
    ...
    def forward(self, obs: torch.Tensor, obs_hist: torch.Tensor) -> torch.Tensor:
        ...
        encode_lin_vel = self.encoder_head_lin_vel(feature)
        z_e = self.encoder_head_codebook(feature)
        z_q = self._quantize(z_e)
        lin_vel_normalized = self.actor_lin_vel_normalizer(encode_lin_vel)
        actor_obs = torch.cat((obs_normalized, lin_vel_normalized, z_q), dim=-1)
        if self.state_dependent_std:
            return self.actor(actor_obs)[..., 0, :]
        else:
            return self.actor(actor_obs)

    def _quantize(self, z_e: torch.Tensor) -> torch.Tensor:
