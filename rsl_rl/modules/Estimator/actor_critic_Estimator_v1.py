...
class ActorCriticEstimatorV1(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        if kwargs:
            print(
                "ActorCriticEstimatorV1.__init__ got unexpected arguments, which will be ignored: " + str([key for key in kwargs])
            )
        super().__init__()
    ...
    def reset(self, dones: torch.Tensor | None = None) -> None:
        self.cenet.encoder.reset(dones)
    ...
    def act(self, obs: TensorDict, bootstrap: bool, stage: Literal["rollout", "update"], masks: torch.Tensor | None = None, hidden_state: HiddenState | None = None) -> torch.Tensor | tuple[torch.Tensor, torch.Tensor]:
        with torch.no_grad():
            curr_obs, history_obs = self.get_actor_obs(obs)
        ...
        encode_lin_vel, encode_feet_clearances, encode_context, _ = self.cenet.encode(history_obs_normalized, masks, hidden_state)
        with torch.no_grad():
            if bootstrap:
    ...
    def get_actor_obs(self, obs: TensorDict) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        curr_obs = obs_concat[..., -1, :].clone()
        history_obs = obs_concat.clone()
    ...
    def get_hidden_states(self) -> HiddenState:
        return self.cenet.encoder.hidden_state
    ...
    def _export_policy_as_onnx(self, obs: TensorDict, model: nn.Module, path: Path) -> None:
        ...
        with torch.no_grad():
            ...
            for p in policy.parameters():
                p.requires_grad_(False)
            obs, obs_hist = self.get_actor_obs(obs)
...
class CENet(nn.Module):
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        # Encoder
        self.encoder = Memory(encoder_input_dim, encoder_hidden_dim, encoder_num_layers, rnn_type)

        # Encoder heads
        for key in encoder_head_hidden_dims:
            if encoder_head_hidden_dims[key] is not None:
                setattr(self, f"encoder_head_{key}", MLP(encoder_hidden_dim, latent_state_dim if "context" in key else estimated_state_dims[key], encoder_head_hidden_dims[key], activation))
            else:
                setattr(self, f"encoder_head_{key}", nn.Linear(encoder_hidden_dim, latent_state_dim if "context" in key else estimated_state_dims[key]))

    def encode(self, obs_hist: torch.Tensor, masks: torch.Tensor | None = None, hidden_states: HiddenState = None) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        feature = self.encoder(obs_hist, masks, hidden_states).squeeze(0)
...
class InferenceWrapper(nn.Module):
    def __init__(self, models: ActorCriticEstimator) -> None:
        super().__init__()
